...
# 1.1.0   | 2026-04-09 | tjuphao [AI-ASSISTED] | Add CppUTest unit test target (make test)
# 1.2.0   | 2026-04-09 | tjuphao [AI-ASSISTED] | Add JUnit XML report output to build/
# 1.3.0   | 2026-04-09 | tjuphao [AI-ASSISTED] | Add lcov coverage target (make coverage)
# 1.4.0   | 2026-04-09 | tjuphao [AI-ASSISTED] | Add MC/DC coverage target (make mcdc)
# 1.5.0   | 2026-04-09 | tjuphao [AI-ASSISTED] | Clean *.gcov files on make clean
# =============================================================================
...
# MC/DC configuration (requires GCC 14+)
MCDC_CC = gcc-14
MCDC_CXX = g++-14
MCDC_CFLAGS = -g -O0 -Wall -Iinclude -I$(CPPUTEST_HOME)/include --coverage -fcondition-coverage
MCDC_LDFLAGS = -L$(CPPUTEST_HOME)/lib -lCppUTest -lCppUTestExt --coverage
MCDC_OUT = build/mcdc_runner

.PHONY: all run test coverage mcdc clean
...
mcdc:
	mkdir -p build
	$(MCDC_CC) $(MCDC_CFLAGS) -c $(TEST_SRC) -o build/hello_mcdc.o
	$(MCDC_CXX) $(MCDC_CFLAGS) build/hello_mcdc.o $(TEST_CPP) $(MCDC_LDFLAGS) -o $(MCDC_OUT)
	./$(MCDC_OUT)
	lcov --capture --directory . --output-file build/mcdc.info \
	     --gcov-tool gcov-14 \
	     --rc branch_coverage=1 --rc mcdc_coverage=1 \
	     --ignore-errors mismatch,mismatch
	lcov --remove build/mcdc.info '/usr/*' '*/test/*' \
	     --output-file build/mcdc_filtered.info \
	     --gcov-tool gcov-14 \
	     --rc branch_coverage=1 --rc mcdc_coverage=1 \
	     --ignore-errors mismatch,mismatch,unused
	genhtml build/mcdc_filtered.info --branch-coverage \
	        --rc mcdc_coverage=1 \
	        --output-directory build/mcdc_report
	@echo "MC/DC report: build/mcdc_report/index.html"

clean:
	rm -rf build gmon.out *.gcov
